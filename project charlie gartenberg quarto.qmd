...
google_trends <- lapply(google_trends_files, function(file) {
  read_lines(file, skip = 1) %>%
...
#  df google trends
google_trends <- bind_rows(google_trends)
...
median_earnings <- median(merged_data$earnings)

# Define high and low
merged_data <- merged_data %>%
  mutate(high_earning_college = ifelse(earnings > median_earnings, 1, 0))

# Stdz Google Trends indices
merged_data <- merged_data %>%
  group_by(keynum) %>%
...
# DID regression
did_model <-
  lm(standardized_index ~ post_scorecard * high_earning_college + control_variables,
     data = merged_data)
...
#GRAPHING
ggplot(merged_data,
       aes(x = monthorweek, y = standardized_index, color = high_earning_college)) +
  geom_line() +
